...
function [h, fig] = kolmogorov_smirnov_test(isi_pdf, isi_probs, N)
    ...
    try
        ...
        cdf_model_unique  = unique(cdf_model);
        cdf_emp_unique    = unique(cdf_empirical);
        min_length        = min(length(cdf_model_unique), length(cdf_emp_unique));
        cdf_model_unique  = cdf_model_unique(1:min_length);
        cdf_emp_unique    = cdf_emp_unique(1:min_length);
        cdf_diff          = interp1(cdf_model_unique,cdf_emp_unique,border_line);
    end
